...
const char SLog::DAY_ROTATION = 'D';
const char SLog::HOUR_ROTATION = 'H';
...
SLog::SLog(struct s_logger_config config){
    ...
    this->cFileRotaPeriod = config.cFileRotaPeriod;
    this->uFileRotaResolution = config.uFileRotaResolution;
...
SLog::SLog(YamlConfig yamlConfig, string pwd, string sLogAppender, bool isOrderLogging)
{
    ...
    if( ! this->isOrderLogging ) {
        ...
        string _x = yamlConfig["logging"]["file_rotation_period"];
        this->cFileRotaPeriod = _x.length() == 1 ? _x.at(0) : DAY_ROTATION;

        this->uFileRotaResolution = atol( yamlConfig["logging"]["file_rotation_resolution"].c_str() );
...
SLog::~SLog()
{
    ...
    _close(fileFd);
#else
    if( logMode == Mode_LIVE_WITH_LOCAL ) {
        delete tcp_client;
        close(fileFd);
...
void SLog::validate()
{
    if( cFileRotaPeriod != DAY_ROTATION && cFileRotaPeriod != HOUR_ROTATION )
        cFileRotaPeriod = DAY_ROTATION;

    if (cFileRotaPeriod == HOUR_ROTATION)
    {
        if ( uFileRotaResolution == 0 || ( ( 24 % uFileRotaResolution)  > 1) )
        {
            cout << "Given File Rotation resolution is not valid, should be a divisor of 24. 0 and 24 is not valid." << endl;
    ...
    if ( cFileRotaPeriod == DAY_ROTATION)
        uFileRotaResolution = 1;

    printf("file_rotation_period:%c, file_rotation_resolution:%u\n",cFileRotaPeriod, uFileRotaResolution);
...
void SLog::make_message(const char *fmt, va_list ap, string & message)
{
    ...
    while(1) {
        ...
        length = vsnprintf(ptr, size, fmt, ap);
...
bool SLog::shift()
{
    ...
    /*
    printf("tm_mday=(%d,%d) tm_hour=(%d,%d)\n", current_tm_time.tm_mday, 
                file_tm_time.tm_mday, 
                current_tm_time.tm_hour, 
                file_tm_time.tm_hour);
    */
    if ( 
            ( cFileRotaPeriod == DAY_ROTATION 
              && current_tm_time.tm_mday != file_tm_time.tm_mday 
            ) 
            || 
            ( cFileRotaPeriod == HOUR_ROTATION 
              && ( current_tm_time.tm_mday != file_tm_time.tm_mday
                   || 
                   current_tm_time.tm_hour >= (file_tm_time.tm_hour + uFileRotaResolution)
                )
            )
       )
    {
        ...
        if ( cFileRotaPeriod == HOUR_ROTATION)
        {
            uint8_t r = file_tm_time.tm_hour / uFileRotaResolution;
            uint8_t l = r * uFileRotaResolution;
            int u = l + uFileRotaResolution;

            char hour[7] = { '\0' };
            sprintf(hour, ".%02u-%02u", l, u);
        ...
        close(fileFd);
#endif
        if ( access(newLogFileName.str().c_str(), F_OK) == -1 )
...
void SLog::log(uint8_t level, string message)
{
    ...
    finalLogMessage.str("");
    finalLogMessage << logMessage.str(); 
}
...
string SLog::getLogMessage(){
    return finalLogMessage.str();
}
