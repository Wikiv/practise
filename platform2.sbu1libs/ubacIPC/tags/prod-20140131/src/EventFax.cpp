...
//			cout << "no of epoll events = " << s << endl;
		for(int i = 0; i < s; i++) {
			if(ev[i].events & EPOLLIN) {
				...
				if( j == clientDescriptorHash.end() ) {
					map<int, UDPClient *>::iterator iUdp = udpClientDescriptorHash.find( ev[i].data.fd );
					if( iUdp == udpClientDescriptorHash.end() ) {
						map<int, TCPServer *>::iterator iServer = serverDescriptorHash.find( ev[i].data.fd );
						if( iServer != serverDescriptorHash.end() ) {
							iServer->second->acceptClientConnection();
						}
					} else {
						if( iUdp->second->readData() <= 0 ) {
							iUdp->second->unbind();
							iUdp->second->closeConnection();
							epoll_ctl(epollFd, EPOLL_CTL_DEL, ev[i].data.fd, &ev[i]);
							udpClientDescriptorHash.erase( iUdp );
						}
					}
				} else {
					if( j->second->readData() <= 0 ) {
...
#ifdef EPOLLRDHUP
			if( ((ev[i].events & EPOLLERR) == EPOLLERR) || ((ev[i].events & EPOLLHUP) == EPOLLHUP) || ((ev[i].events & EPOLLRDHUP) == EPOLLRDHUP) ) {
#else
			if( ((ev[i].events & EPOLLERR) == EPOLLERR) || ((ev[i].events & EPOLLHUP) == EPOLLHUP) ) {
#endif
				cout << "EPOLLERR Event for i as " << i << " with fd as " << ev[i].data.fd << endl;
...
//	int ret;
	epoll_ctl (epollFd, EPOLL_CTL_ADD, clientFd, (struct epoll_event *)&tcp_server->epollEventData);
...
void EventFax::deleteTCPClient(TCPClient *client)
{
	...
	map<int, TCPClient *>::iterator iClient = clientDescriptorHash.find( clientFd );
	if( iClient != clientDescriptorHash.end() ) {
#ifndef WIN32
		int ret = epoll_ctl (epollFd, EPOLL_CTL_DEL, clientFd, (struct epoll_event *)&iClient->second->epollEventData);
...
#ifdef EPOLLRDHUP
	tcp_client->epollEventData.events = tcp_client->epollEventData.events | EPOLLRDHUP;
#endif
	tcp_client->epollEventData.data.fd = clientFd;
	clientDescriptorHash.insert( make_pair(clientFd, tcp_client) );
//	cout << "epollFd = " << epollFd << ", fd = " << clientFd << endl;
	int ret = epoll_ctl (epollFd, EPOLL_CTL_ADD, clientFd, (struct epoll_event *)&tcp_client->epollEventData);
	if( ret != 0 ) {
//		cout << "ret for adding client = " << ret << endl;
//		cout << "errno = " << strerror( errno ) << endl;
	}
...
#ifndef WIN32
void EventFax::addUDPClient(UDPClient *udp_client)
{
	cout << "adding UDP client" << udp_client->descriptor <<  endl;
	int clientFd = udp_client->descriptor;
	int val = fcntl (clientFd, F_GETFL, 0);
	fcntl (clientFd,F_SETFL, val | O_NONBLOCK);
	udp_client->epollEventData.events = EPOLLIN | EPOLLET | EPOLLERR;
	udp_client->epollEventData.data.fd = clientFd;
	udpClientDescriptorHash.insert( make_pair(clientFd, udp_client) );
	cout << "epollFd = " << epollFd << ", fd = " << clientFd << endl;
	int ret = epoll_ctl (epollFd, EPOLL_CTL_ADD, clientFd, (struct epoll_event *)&udp_client->epollEventData);
	if( ret != 0 ) {
			cout << "ret for adding client = " << ret << endl;
			cout << "errno = " << strerror( errno ) << endl;
	}
