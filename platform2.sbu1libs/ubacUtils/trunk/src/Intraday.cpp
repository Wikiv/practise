...
uint8_t Intraday::compute(const QuoteData & quoteData,
	...
	map<string, IntradayData>::iterator iCache = this->cache.find(
			quoteData.sSymbol);
	...
	if (iCache == this->cache.end()) {
		...
		intradayData.lTime = quoteData.lLastTradedTime
				+ (this->seconds - (quoteData.lLastTradedTime % this->seconds));
		...
		time_t _t = (time_t) intradayData.lTime;
		localtime_r(&_t, &oldTime);

		time_t tNewTime = quoteData.lLastTradedTime
				+ (this->seconds - (quoteData.lLastTradedTime % this->seconds));
...
void Intraday::clearCache() {
	cache.clear();
}

map<string, IntradayData> Intraday::getCache() {
	return cache;
}
