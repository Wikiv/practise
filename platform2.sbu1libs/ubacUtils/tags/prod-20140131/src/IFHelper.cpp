...
void IFHelper::gsub(string & data, string strFrom, string strTo)
{
	...
	newsplit(data, strFrom, splitList);
...
/*void IFHelper::htmlToText(string & error)
{
	string tmp;
	bool isHref = false;
...
void IFHelper::newsplit(string data, string pattern, vector<string> & list)
{
...
{
	char *c_data = (char *)calloc(1, data.length() +1);
	const char *c_pattern = pattern.c_str();
	memcpy(c_data, data.c_str(), data.length());
	char *c_tmp_data = c_data;
	string sToken;

	while(1) {
		char *c_tmp = strstr(c_tmp_data, c_pattern);
		if( c_tmp == NULL ) {
			sToken.clear();
			sToken.append(c_tmp_data);
			list.push_back(sToken);
			break;
		} else {
			sToken.clear();
			sToken.append(c_tmp_data, strlen(c_tmp_data) - strlen(c_tmp));
			list.push_back(sToken);
		}
		c_tmp_data = c_tmp;
		c_tmp_data += strlen(c_pattern);
		if( *c_tmp_data == '\0' )
			break;
	}
	free(c_data);
	/*
	string buffer;
	size_t lengthOfData = data.length(), sizeOfPattern = pattern.length(), i = 0;
	for(; (i+sizeOfPattern) < lengthOfData; ) {
		string tmp;
		for(size_t j = 0; j < sizeOfPattern; j++)
			tmp.append(&data[i+j], 1);
		if( strncmp(tmp.c_str(), pattern.c_str(), sizeOfPattern) == 0 ) {
			if( buffer.length() > 0 ) {
	...
	buffer.append(data, i, i+sizeOfPattern - 1);
	list.push_back( buffer );
	*/
}
...
string IFHelper::getPWD(const char *c_progname, const char *c_path)
{
	...
	vector<string>sList;
	newsplit(sData, string(c_path), sList);
	sPWD << sList[0] << "/";
