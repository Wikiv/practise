...
const uint16_t Cipher::Cipher_TRIPLE_DES_CBC = 5;
...
void Cipher::decrypt(uint16_t u16_cipher_type, unsigned char *input, size_t input_length, unsigned char *output, size_t *outputLength, int padding)
{
	...
	if( u16_cipher_type == Cipher_TRIPLE_DES_CBC ) {
		int op_len = 0;
        cout << "\nKEY=" << key;
		EVP_DecryptInit_ex(&ctx, EVP_des_ede3_cbc(), NULL, key, iv);
                cout << endl << (char *)input;
		EVP_CIPHER_CTX_set_padding(&ctx, padding);
		EVP_DecryptUpdate(&ctx, output, (int *)&op_len, input, (int)input_length);
                *outputLength = op_len;
                EVP_DecryptFinal_ex(&ctx, output+op_len, (int *)&op_len);
                *outputLength += op_len;
	}
...
void Cipher::encrypt(uint16_t u16_cipher_type, unsigned char *input, size_t input_length, unsigned char *output, size_t *outputLength, int padding)
{
	...
	if( u16_cipher_type == Cipher_DES_CBC ) {
		...
		*outputLength = op_len;
		...
		EVP_CipherFinal_ex(&ctx, &output[op_len], &op_len2);
		*outputLength += op_len2;
    } else if( u16_cipher_type == Cipher_TRIPLE_DES_CBC ) { 
        size_t op_len = 0;
