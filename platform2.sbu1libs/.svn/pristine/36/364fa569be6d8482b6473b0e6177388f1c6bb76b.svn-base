...
bool MultiCastClient::connect(string groupIP, int port)
{
	...
	if (iUDPSocket == -1){
		...
		//cout <<" port: " << port << endl;
	        int retcode = bind(iUDPSocket,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
        	...
        	if (retcode < 0) {
        		...
        		// subsribing to multicast group
		       	retcode = setsockopt(iUDPSocket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&mreq, sizeof(mreq));
        		if (retcode < 0){
        			// throw some error..
	            		iUDPSocket = -1;
	        		return false;
...
void MultiCastClient::start()
{
	while(1) {
		...
		int received_bytes = recvfrom(iUDPSocket, buffer, sizeof(buffer), 0, (struct sockaddr *) &client_addr, (unsigned int *) &addr_len);
