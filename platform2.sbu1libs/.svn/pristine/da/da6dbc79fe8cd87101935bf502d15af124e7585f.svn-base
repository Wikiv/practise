...
void EventFax::run()
{
	 ...
	 while(1) {
		  ...
		  vector<FileDescriptor *> vEventFileDesc;
			   ...
			   if(ev[i].events & EPOLLIN) {
				    ...
				    if ( _r <= 0 ) {
					     ...
					     if ( fileDesc->isDataAvailForRead() )
					     {
						      ...
						      vEventFileDesc.push_back(fileDesc);
					     }
				    }
			   }
...
void EventFax::checkReadEvent(vector<FileDescriptor *> & vEventFileDesc)
{
	 vector<FileDescriptor *>::iterator it = this->vReadFileDesc.begin();

	 while ( it != this->vReadFileDesc.end() )
	 {
		  FileDescriptor * fd = * it;

		  if ( std::find(vEventFileDesc.begin(), vEventFileDesc.end(), fd) 
			!= vEventFileDesc.end())
		  {
			// Skiping the FileDescriptor which were already processed with read events.
			it++;
			continue;
		  }
		  ...
		  if ( fd->isDataAvailForRead() )
		  {
			   ...
			   it = this->vReadFileDesc.erase(it);
		  }
