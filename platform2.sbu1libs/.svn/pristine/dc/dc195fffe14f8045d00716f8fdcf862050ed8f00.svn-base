...
TCPServer::TCPServer(int port, int listen_q) : FileDescriptor(0)
{
    descriptor = -1;
...
TCPServer::~TCPServer()
{
    if (descriptor > 0)
    {
        ...
        close(descriptor);
#endif
        cout << "Closing descriptor" << endl;
    }
}

int TCPServer::handleIN()
{
...
int TCPServer::startServer(int port, int listen_q)
{
    ...
    #ifdef SO_REUSEPORT
    if (setsockopt(descriptor, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt))==-1) {
        close(descriptor);
        descriptor = -1;
    }
    #endif
