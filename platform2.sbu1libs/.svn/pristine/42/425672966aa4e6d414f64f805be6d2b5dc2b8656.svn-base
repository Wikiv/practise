...
void IFHelper::gsub(string & input, string from, string to)
{
/*
	string tmpData;
...
*/
    ...
    while((f = input.find(from, f)) != string::npos) {
        input.replace(f, from.length(), to ) ;
        f+= to.length();
    }
}
...
void IFHelper::logTime(string & formattedTime)
{
    ...
    snprintf(c_micro_secs, sizeof(c_micro_secs), ".%06lu", tv.tv_usec);
...
{
    char buffer[1024] = {'\0'};
    size_t len = snprintf(buffer, sizeof(buffer), "%f", d_value);
    size_t j = len;
...
#else
    unsigned int length = snprintf(tmpFormattedOutput, sizeof(tmpFormattedOutput), formatStream.str().c_str(), d_value);
#endif

    if(length > sizeof(tmpFormattedOutput) || (length < 0)) {
        cout << "big size error\n";
        return;
    }
