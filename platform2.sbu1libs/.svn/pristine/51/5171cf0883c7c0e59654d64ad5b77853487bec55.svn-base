...
{
	time_t t = time(NULL);
	map<time_t, vector<struct ubac_timed_event_t> >::iterator iEvent =
		timed_events.begin();
...
//		cout << "first = " << iEvent->first << ", Current = " << t << endl;
		...
		vector<struct ubac_timed_event_t>::iterator iEventList =
			iEvent->second.begin();
...
void TimerEvents::addPeriodicEvent( void (*f)(void *), void *data,
									uint32_t timer)
{
