...
const uint8_t SBU2StreamingRequest::SBU2StreamingRequest_MAX_SYMS = 100;

SBU2StreamingRequest::SBU2StreamingRequest(string sRequest)
{
	this->subscription = this->unSubscription = this->valid = false;
...
void SBU2StreamingRequest::setAppID(){
    JsonObject req =  reqObject["request"];
    if(req.hasKey("appID")){
        sAppID = req["appID"];
    }
}
...
void SBU2StreamingRequest::parse(string sRequest)
{
	...
	valid = true; //Setting request as valid
	...
	if( vSymbol.size() >= SBU2StreamingRequest_MAX_SYMS ){
		valid = false;
		return;
	}
